...
/// Gap 4-recipient: drive one scheduler tick. The Tauri shell calls
/// this on a 60 s background timer (see `src-tauri/src/lib.rs`
/// `setup()`) so per-row subscription cadence in `scheduler.Plan`
/// actually fires while the GUI is running.
pub fn scheduler_tick(state: &AppState) -> Result<()> {
