...
**Status:** locked at FRP-10 (May 2026). · **Supplement reference:** §9.5.2, §9.5.5. · **Engine line:** `daal-core 0.9.0+v3-share` (no engine ABI surface — this is an in-box userspace daemon).

This spec defines the contract between the FRP Helper and the persistent in-box `daal-relay-mgmt` service that ships with FRP-10's V2 deploys. It locks the wire format, the auth scheme, the TLS posture, the port discipline, and the three-route API surface. The Helper-side mirror (`publisher/deploy/mgmt/`) and the box-side service (`cmd/daal-relay-mgmt/`) are both pinned by tests against this contract.
...
* `op` is one of `rotate-credentials` or `rotate-tls`. The on-box service rejects any token whose `op` does not match the URL path.
...
## 4. API surface — exactly three routes

Adding a fourth route requires a supplement amendment. `cmd/daal-relay-mgmt/main_test.go::TestExactlyThreeRoutes` enforces this at the implementation level.
...
| File | Purpose |
|---|---|
| `cmd/daal-relay-mgmt/main_test.go` | 11 tests pinning the wire format, the three-route invariant, the Ed25519 token shape (op-binding + timestamp window), the self-signed cert fingerprint stability across restarts, and the surgical sing-box JSON rewriters |
