...
export default function AddEntryModal({ t, mode, onClose }: Props) {
    ...
    const onPreview = async () => {
        ...
        try {
            let target = filePath;
            // Device Custody v1: .sbpx unwrap uses the recipient
            // X25519 priv held in device custody — no PIN at this
            // layer.
            if (filePath && (await Sbpx.sniff(filePath))) {
                target = await Sbpx.import(filePath);
    ...
    const onImport = async () => {
        ...
        try {
            if (mode === 'route') {
                ...
                // Gap 4-recipient bug fix: engine rejects empty
                // publisher_fingerprint. See lib/subscriptionIdentity.ts.
                ...
                {tab === 'file' && (
                    <div>
                        <input
                            ...
                            style={inputStyle()}
                        />
                        {/* Device Custody v1: no PIN at the recipient
                            layer. The X25519 priv that unwraps the
                            envelope is held in device custody
                            (hardware / OS keystore / session
                            passphrase). */}
                        <button
